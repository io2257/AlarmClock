...
class InfoFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pointManager = PointManager(requireContext())
    }
                ...
                BiFunction { next, now ->
                    next.getOrNull()?.let {
                        computeTexts(
                            ...
                            pointManager.getPoints() // 여기서 현재 포인트를 전달
                        )
                    }
                        ?: ""
                })
...
fun computeTexts(
    ...
    currentPoints: Int = 0 // 새로운 매개변수 추가
): String {
    ...
    return if (alarm.isPrealarm) {
       ...
       $prealarmSummary
       현재 $currentPoints 포인트를 모았습니다!"""
            .trimIndent()
    } else {
        "$formatRemainingTimeString before occurring next alarm\n현재 $currentPoints 포인트를 모았습니다!"
    }
}
